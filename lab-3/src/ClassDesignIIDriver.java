 ...
 * On observing the execution of the demo code, answer the following questions...
	...
	// ANS: The primitive value is copied, the reference value is copied.
	*
    When a primitive is passed to a method, does the scope of that primitive change (i.e., grow to include
    * the called method)?
	// ANS: In a sense, the primitive itself is copied to the method. In the context of the copied primitive
	* in the new method, it's scope is expanded, however the scope of the original primitive remains the same.
	*
    What about when an object is passed to a method?
	// ANS: The reference value is copied, which refers to the original object. In this sense, the original scope
	* is expanding by the distribution of the reference value in memory being expanded to include additional functions.
	*
    How then would you describe the scope of an object that has been passed to a method?
	// ANS: The scope of the object passed to the method is expanded to include the method that it is passed into.
 */


public class ClassDesignIIIDriver{
